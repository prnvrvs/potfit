...
def decorate(func):
    def wrapped(self=None, desc=""):
        return "\n".join([func(self, s).rstrip() for s in desc.split("\n")])
...
OPTIONS = [
    ...
    [
        "dsf",
        "Use damped shifted force approach (coulomb-based interactions only)",
        ["DSF"],
    ],
...
INTERACTIONS = [
    ...
    [
        "ang_elstat",
        "angular pair potentials with eletrostatics",
        ["apot", "tab"],
        ["ANG", "COULOMB"],
        ["force_ang_elstat.c"],
    ],
    ...
    [
        "eam_coulomb",
        "embedded atom method with coulomb interactions",
        ["apot", "tab"],
        ["EAM", "COULOMB"],
        ["force_eam_elstat.c"],
    ],
    [
        "eam_dipole",
        "embedded atom method with dipole interactions",
        ["apot", "tab"],
        ["EAM", "COULOMB", "DIPOLE"],
        ["force_eam_elstat.c"],
    ],
...
def options(opt):
    ...
    """
    opt.load("compiler_c")
    opts = opt.add_option_group(
        "potfit general options",
    ...
    pots = opt.add_option_group(
        "potfit potential options",
        "available interactions in alphabetical order are:\n{}".format(
            il.get_interaction_desc()
        ),
    )
    pots.add_option(
        "-i",
        ...
        action="store",
        type=str,
        help="one of the interactions listed above",
        metavar="INTERACTION",
    )
    pots.add_option(
        "-m",
        ...
        action="store",
        type=str,
        help="support analytic, kim or tabulated potentials",
    )
    # math libraries
    libs = opt.add_option_group(
        "potfit math library options",
        "available math libraries are:\n{}".format(
            "\n".join(
                sorted(["\t{:<16}{}".format(x, v) for x, v in supported_math_libs])
            )
        ),
    )
    libs.add_option(
        "-l",
        "--math-lib",
        default="acc" if _platform == "darwin" else "mkl",
        choices=[name for name, _ in supported_math_libs],
        help="Select math library to use (default: %default)",
        metavar="MATHLIB",
    )
    libs.add_option(
        "--math-lib-base-dir",
    ...
    debug.add_option(
        ...
        action="store_true",
        default=False,
        help="Build binary with debug information",
    )
    debug.add_option(
        "--asan",
        action="store_true",
    ...
    debug.add_option(
        ...
        action="store_true",
        default=False,
        help="Build binary with profiling support",
    )
...
def configure(cnf):
    ...
    if cnf.options.model != "kim":
        ...
        cnf.env.append_value(
            "DEFINES_POTFIT", il.get_interaction(cnf.options.interaction).defines
        )
    ...
    cnf.define("POTFIT_VERSION", _get_version_string())
    ...
    opts = [
        x[7:]
        for x in dir(cnf.options)
        if x.startswith("enable_") and getattr(cnf.options, x)
    ]
    if len(opts):
        for i in opts:
            for j in OPTIONS:
...
def _check_potential_options(cnf):
    if cnf.options.model not in ["apot", "kim", "tab"]:
        cnf.fatal(
            "Invalid model specified, please provide -m/--model from 'apot', 'kim' or 'tab'"
        )
    ...
    if not interaction.supports_model(cnf.options.model):
        cnf.fatal(
            "Interaction {} does not support model {}!".format(
                interaction.name, cnf.options.model
            )
        )
...
def _check_enable_options(cnf):
    ...
    if cnf.options.enable_dsf and cnf.options.interaction not in [
        "ang_elstat",
        "coulomb",
        "eam_coulomb",
    ]:
        cnf.fatal(
            "DSF can only be used with COULOMB-based interactions and not with {}".format(
                cnf.options.interaction
            )
        )
    ...
    if cnf.options.enable_bindist and cnf.options.model == "apot":
        cnf.fatal(
            "Binned radial distribution files are not available for analytic potentials."
        )
    ...
    if cnf.options.enable_uq:
        if cnf.options.model != "apot":
            cnf.fatal(
                "Uncertainty quantification is only supported for analytic potentials!"
            )
...
def _check_compiler_options(cnf):
    ...
    if cnf.options.model == "kim":
        cnf.check_cfg(
            package="libkim-api",
            args=["libkim-api >= 2.0.2", "--cflags", "--libs"],
            uselib_store="KIM",
        )
    else:
    ...
    if cnf.options.enable_mpi:
        if cnf.options.check_c_compiler:
            cnf.fatal(
                "--check-c-compiler cannot be used with MPI! "
                "Please use the MPI environment variable to set the compiler."
            )
        cnf.add_os_flags("CC")
        if cnf.env.CC:
            cnf.fatal(
                "Overriding the compiler with CC is not supported when MPI is enabled!"
            )
        else:
            cnf.env.CC = "mpicc"
    ...
    if cnf.env.CC_NAME == "icc":
        if cnf.options.debug:
            ...
            cnf.env.append_value(
                "CFLAGS_POTFIT", ["-fast", "-xHost", "-std=c99", "-fPIC"]
            )
    ...
    if _platform == "darwin":
        ...
        cnf.env.append_value(
            "LINKFLAGS_POTFIT", ["-Wl,--no-undefined,--as-needed,-z,relro,-z,now"]
        )
...
def _check_math_lib_options(cnf):
    ...
    if cnf.options.math_lib == "mkl":
        ...
        try:
            ...
            cnf.check(
                header_name="mkl_lapack.h", features="c cprogram", use=["POTFIT", "MKL"]
            )
            cnf.check_cc(
                fragment="""#include <mkl_vml.h>
...
def _get_version_string():
    try:
        process = run(
            ["git", "log", "-1", "--oneline", '--pretty="%h (%cs)"', "HEAD"],
            check=True,
            capture_output=True,
        )
        head = process.stdout.decode().replace('"', "").strip()
        process = run(
            ["git", "log", "--oneline", '--pretty="%h (%cs)"', "HEAD...origin~1"],
            check=True,
            capture_output=True,
        )
        origin = process.stdout.decode().replace('"', "").strip().split('\n')
        if len(origin) > 1:
            head += " origin " + origin[-1]
        return head
    except CalledProcessError:
        return POTFIT_VERSION
...
class OptList:
    ...
    def get_opt_desc(self):
        return "\n".join(
            sorted(["\t{:<16}{}".format(i.name, i.description) for i in self.list])
        )
